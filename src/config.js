...
// Order:
//   1) CYPHEREDGE_USER_DATA override
//   2) EMAIL_ENV/BUILD_ENV/APP_ENV = 'dev' -> repo frontend (if exists)
//   3) Non-packaged dev fallback -> repo frontend (if exists)
//   4) Default -> %APPDATA%/CypherEdge
const getCypherEdgeDataPath = () => {
  ...
  const frontendPath = path.resolve(
    __dirname,
    "../../beta_testers_ca/frontend"
  );
  const fs = require("fs");

  // Explicit dev env: align with Electron dev repo path
  if (buildEnv === "dev" && fs.existsSync(frontendPath)) {
    return frontendPath;
  }

  // Non-packaged dev fallback
  if (!isPackaged && fs.existsSync(frontendPath)) {
    return frontendPath;
  }
...
module.exports = {
  ...
  serverBaseUrl:
    ...
    `http://localhost:${process.env.PORT || 8234}`,

  // Scan settings
  // Default to 30 days; override via INITIAL_SCAN_DAYS if needed
  initialScanDays: parseInt(process.env.INITIAL_SCAN_DAYS || "30", 10),
